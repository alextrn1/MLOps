...
export function ModelsRegistryPage() {
  ...
  const { data: modelData, error, loading, retry } = useCachedResource<ModelDto[]>("models:list", () => modelsApi.listModels(), []);
  ...
  return <section className="models-page models-registry-page"><div className="models-page-heading"><div><h1>Реестр моделей</h1><p>Централизованное хранилище всех ML-моделей платформы</p></div><button className="ui-button ui-button--primary register-model-button" type="button" onClick={() => navigate("/models/new")}><AppIcon name="plus" size={18} aria-hidden />Регистрация модели</button></div>
    <div className="models-toolbar"><div className="models-search"><AppIcon name="search" size={18} aria-hidden /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск по названию или описанию…" aria-label="Поиск моделей" /></div><button type="button" className={`ui-button ui-button--secondary filter-button ${filtersOpen ? "filter-button--active" : ""}`} onClick={() => setFiltersOpen((value) => !value)} aria-expanded={filtersOpen}><AppIcon name="filter" size={18} aria-hidden />Фильтры</button></div>
    {error ? <ErrorState title="Не удалось загрузить модели" description="Проверьте подключение к API и попробуйте снова." onRetry={retry} /> : null}
    {!error && models.length === 0 ? <DelayedLoadingState loading={loading} label="Загружаем реестр моделей…" /> : null}
