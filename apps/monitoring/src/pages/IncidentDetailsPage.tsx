...
export function IncidentDetailsPage() {
  ...
  return <section className="monitoring-page incident-detail">
    ...
    <div className="incident-detail-bottom"><article className="incident-environment-card"><h2>Детали среды</h2><div><span><AppIcon name="server" size={15} aria-hidden />Эндпоинт</span><Link to={`/deployments/${incident.deployment.id}`}>{incident.deployment.url}</Link><b>{incident.deployment.environment}</b></div><div><span>Проект</span><Link to={`/projects/${incident.project.id}`}>{incident.project.name}</Link></div></article><article className="incident-actions-card"><h2>Действия</h2>{incident.status === "resolved" ? <div className="incident-closed"><div><AppIcon name="success" size={20} aria-hidden />Инцидент закрыт</div><span>Кем закрыт</span><strong>{incident.resolvedBy}</strong><span>Время закрытия</span><strong>{incident.resolvedAt ? formatDateTime(incident.resolvedAt) : "—"}</strong></div> : null}{incident.status !== "resolved" || incident.observabilityUrl ? <div className="incident-action-buttons">{incident.status === "open" ? <Button className="incident-acknowledge" type="button" disabled={action !== null} onClick={() => runAction("acknowledge")}>{action === "acknowledge" ? "Сохранение…" : "Взять в работу (Acknowledge)"}</Button> : null}{incident.status !== "resolved" ? <Button className="incident-resolve" type="button" disabled={action !== null} onClick={() => runAction("resolve")}>{action === "resolve" ? "Сохранение…" : "Отметить как решённый"}</Button> : null}{incident.observabilityUrl ? <a className="ui-button ui-button--secondary" href={incident.observabilityUrl} target="_blank" rel="noreferrer">Перейти в Grafana / Логи</a> : null}</div> : null}</article></div>
  </section>;
}
