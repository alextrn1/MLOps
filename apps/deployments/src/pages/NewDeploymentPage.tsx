...
import { deploymentEnvironmentOptions, deploymentProjectOptions, getDeploymentModelOptions, getDeploymentVersionOptions } from "../deploymentFormOptions";
...
const initialProjectId = deploymentProjectOptions[0]?.value ?? "";
const initialModelId = getDeploymentModelOptions(initialProjectId)[0]?.value ?? "";
const initial: FormModel = { name: "", environment: "production", url: "", projectId: initialProjectId, modelId: initialModelId, modelVersionId: getDeploymentVersionOptions(initialModelId).find((version) => version.label === "v2.1.0")?.value ?? "", trafficPercent: "100" };
...
export function NewDeploymentPage() {
  ...
  const update = (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((current) => {
      if (name === "projectId") {
        const modelId = getDeploymentModelOptions(value)[0]?.value ?? "";
        return { ...current, projectId: value, modelId, modelVersionId: getDeploymentVersionOptions(modelId)[0]?.value ?? "" };
      }
      if (name === "modelId") return { ...current, modelId: value, modelVersionId: getDeploymentVersionOptions(value)[0]?.value ?? "" };
  ...
  const modelOptions = getDeploymentModelOptions(form.projectId);
  const versionOptions = getDeploymentVersionOptions(form.modelId);
