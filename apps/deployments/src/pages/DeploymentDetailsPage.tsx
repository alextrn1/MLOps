  ...
  return <section className="deployments-page deployment-detail">
    ...
    <div className="deployment-detail-grid">
      ...
      <aside className="deployment-side-column">
        <article className="deployment-side-card deployment-relations"><h2>Связи</h2><div><span>Проект</span><a href={`/projects/${deployment.project.id}`}>{deployment.project.name}</a></div><div><span>Модель</span><a href={`/models/${deployment.model.id}`}>{deployment.model.name}</a><b className="deployment-version">{deployment.modelVersion}</b></div></article>
        <article className="deployment-side-card deployment-events"><h2><AppIcon name="alert" size={23} aria-hidden />События</h2>{events.length ? <div className="deployment-events-list">{events.map((event) => event.monitoringEventId ? <a href={`/monitoring/${event.monitoringEventId}`} key={event.id}><i aria-hidden /><strong>{event.title}</strong><span>{new Date(`${event.occurredAt}T00:00:00`).toLocaleDateString("ru-RU")} • {event.type}</span></a> : <div className="deployment-event-static" key={event.id}><strong>{event.title}</strong><span>{new Date(`${event.occurredAt}T00:00:00`).toLocaleDateString("ru-RU")} • {event.type}</span></div>)}</div> : <p className="deployment-events-empty">Инцидентов не найдено</p>}</article>
      </aside>
    </div>
  </section>;
}
